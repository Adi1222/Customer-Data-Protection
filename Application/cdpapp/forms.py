...
class CustomerForm(forms.ModelForm):
    class Meta:
        model = Cust_org
        widgets = {'onboard_date': DateInput()}
        fields = ['cust_org', 'cust_org_acro', 'status', 'bill_plan', 'onboard_date']
...
class EditUserForm1(UserChangeForm):
    password = None
    class Meta:
        model = User
        fields = ('email', 'username', 'first_name', 'last_name')

class EditUserForm2(UserChangeForm):
    password = None
    class Meta:
        model = Appuser
        fields = ['designation']
