...
def get_camera_tampering_data(request, *args, **kwargs):
    ...
    if request.is_ajax():
        ...
        print(6)
        print(cust_inst)
        agents = Agent.objects.get(id=agentid)
        print(7)
        ...
        while start <= end :
            ...
            incidents = Incident.objects.filter(incident_date=start, agent_id=agentid, category=category)
            total = 0
            labels.append(start)
...
@csrf_exempt
def get_mobile_detected_data(request, *args, **kwargs):
    labels = []
    data = []
...
@csrf_exempt
def get_multiple_people_data(request, *args, **kwargs):
    labels = []
    data = []
...
@csrf_exempt
def get_unidentified_person_data(request, *args, **kwargs):
    labels = []
    data = []
